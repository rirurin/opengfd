...
public class PublishState : IDisposable
{
    ...
    {
        get => Path.Combine(RootPath, "Publish", Package, "ToUpload");
    }
    // User: Publish Output
    public string PublishBuildDirectory // Build directory for current version of the mod.
    {
        get => Path.Combine(RootPath, "Publish", Package, "Builds", "CurrentVersion");
    }
    ...
    public PublishState(string _RootPath, string _Package, bool IsPublishing)
    {
        RootPath = _RootPath;
        Package = _Package;
        if (IsPublishing)
    ...
    {
        if (!Directory.Exists(ReloadedToolsPath) || Directory.GetFiles(ReloadedToolsPath).Length == 0)
        {
            Console.WriteLine($"{new ColorRGB(237, 66, 155)}FAILED{new ClearFormat()}");
            throw new Exception("Reloaded Tools do not exist in the Publish directory!");
        }
    }

    public void CreateArtifacts(string PackageName)
    {
...
public abstract class PublishCommon
{
    ...
    public void Publish(string PackageName)
    {
        ...
        using (var toolPublish = new Process())
        {
            ...
            toolPublish.StartInfo.Arguments += $"--packagename \"{PackageName}\" ";
